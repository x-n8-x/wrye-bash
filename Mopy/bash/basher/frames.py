...
class _BashFrame(wx.Frame):
    _frame_settings_key = None
    _size_hints = _def_size = (250, 250)

    def __init__(self, *args, **kwargs):
        _key = self._frame_settings_key
        if _key is not None:
            kwargs['pos'] = bass.settings.get(_key + '.pos', balt.defPos)
            kwargs['size'] = bass.settings.get(_key + '.size', self._def_size)
        super(_BashFrame, self).__init__(*args, **kwargs)
...
#------------------------------------------------------------------------------
class InstallerProject_OmodConfigDialog(_BashFrame):
